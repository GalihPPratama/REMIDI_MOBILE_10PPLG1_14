...
public class FragmentLaliga extends Fragment {
    ...
    private RecyclerView recyclerViewSpain;

    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstance) {
        View view = inflater.inflate(R.layout.fragment_laliga, container, false);
        recyclerViewSpain = view.findViewById(R.id.recyclerviewLiga);
        recyclerViewSpain.setLayoutManager(new LinearLayoutManager(getContext()));
        loadTeams();
        return view;
    }
    ...
    private void loadTeams(){
        RetrofitClient.getRetrofitInstance()
                ...
                .getAllTeams2()
                .enqueue(new Callback<laligaResponse>() {
                    @Override
                    public void onResponse(Call<laligaResponse> call, Response<laligaResponse> response) {
                        if (response.isSuccessful() && response.body() != null) {
                            ...
                            SpanishAdapter SpanishAdapter = new SpanishAdapter(requireContext(), teams);
                            recyclerViewSpain.setAdapter(SpanishAdapter);
                        } else {
